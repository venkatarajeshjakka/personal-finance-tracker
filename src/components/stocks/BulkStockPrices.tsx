...
const BulkStockPrices: React.FC<BulkStockPricesProps> = ({
  ...
  const getMarketStatusDisplay = () => {
    const statusConfig = {
      OPEN: { color: 'text-green-600', bg: 'bg-green-50', text: 'Market Open' },
      CLOSED: { color: 'text-gray-600', bg: 'bg-gray-50', text: 'Market Closed' },
      PRE_MARKET: { color: 'text-blue-600', bg: 'bg-blue-50', text: 'Pre-Market' },
      POST_MARKET: { color: 'text-orange-600', bg: 'bg-orange-50', text: 'Post-Market' },
      HOLIDAY: { color: 'text-red-600', bg: 'bg-red-50', text: 'Market Holiday' }
    };
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          {showRefresh && (
            <button
              ...
              disabled={loading}
              className="flex items-center gap-2 px-3 py-1 text-sm bg-blue-50 hover:bg-blue-100 text-blue-700 rounded-md transition-colors disabled:opacity-50"
        ...
        {stats.total > 0 && (
          <div className="flex items-center gap-4 text-sm text-gray-600">
            ...
            {stats.cached > 0 && (
              <span className="text-blue-600">Cached: {stats.cached}</span>
            )}
          </div>
        )}
      </div>
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="text-right min-w-[120px]">
                {company.priceError ? (
                  ...
                  <div className="text-sm text-gray-400">No symbol</div>
                ) : loading ? (
